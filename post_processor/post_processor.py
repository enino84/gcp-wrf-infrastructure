...
Usage (called by run_postprocess.sh):
  python post_processor.py --input /data --context "Colombia" --output /output
"""
...
# ============================================================
# LOAD DATA (robust)
# - Avoid open_mfdataset (netCDF4/HDF locking issues)
# - Skip single-snapshot files by default (Time < 2)
# - Optionally copy to safe filenames (':' -> '-') inside /output/tmp_nc
# ============================================================
...
def open_one(path: str) -> xr.Dataset:
    # decode_times=False because WRF Times is a char array and we parse it manually
    return xr.open_dataset(path, engine="netcdf4", decode_times=False)


def safe_copy_to_output_tmp(src_files: list[str], out_dir: Path) -> list[str]:
    """
    Copy wrfout files into OUTPUT_DIR/tmp_nc with ':' replaced by '-'.
    This avoids potential tooling issues with ':' in filenames.
    Uses /output (bind mount) instead of /tmp to avoid tmpfs limits.
    """
    tmp_dir = out_dir / "tmp_nc"
    tmp_dir.mkdir(parents=True, exist_ok=True)

    dst_files: list[str] = []
    for src in src_files:
        srcp = Path(src)
        safe_name = srcp.name.replace(":", "-")
        dst = tmp_dir / safe_name

        # Only copy if missing OR size differs (cheap correctness check)
        if not dst.exists() or dst.stat().st_size != srcp.stat().st_size:
            print(f"  Copying {srcp.name} → {safe_name}")
...
print(f"Found {len(files_orig)} wrfout file(s). Probing timesteps...")
probe_info: list[tuple[str, int]] = []
for f in files_orig:
    try:
        d = open_one(f)
        nt = int(d.sizes.get("Time", 0))
        d.close()
        probe_info.append((f, nt))
        print(f"  {Path(f).name}: Time={nt}")
    except Exception as e:
        # Mark unreadable files so they get filtered out
        probe_info.append((f, -1))
        print(f"  WARNING: cannot open {Path(f).name}: {e}")

# By default: require at least 2 timesteps so accumulations/animations make sense.
# If you want to include single snapshots, set MIN_TIMESTEPS=1.
MIN_TIMESTEPS = 2

kept = [f for (f, nt) in probe_info if nt >= MIN_TIMESTEPS]
if not kept:
    # Fallback: keep any readable file with Time>=1
    kept = [f for (f, nt) in probe_info if nt >= 1]
    print("WARNING: no files with Time>=2; falling back to Time>=1.")

if not kept:
    print("ERROR: no usable wrfout files (all unreadable or empty Time dimension).")
    sys.exit(2)

print(f"Using {len(kept)} wrfout file(s):")
for f in kept:
    print(f"  ✓ {Path(f).name}")

# Optional copy to safe names. If you want zero-copy, set this False.
USE_SAFE_COPY = True
files_to_open = safe_copy_to_output_tmp(kept, OUTPUT_DIR) if USE_SAFE_COPY else kept

# Open sequentially and concat along Time (robust)
datasets: list[xr.Dataset] = []
for f in files_to_open:
    print(f"  Opening: {f}")
    datasets.append(open_one(f))

ds = xr.concat(datasets, dim="Time")


# ============================================================
# Coordinates, times, metadata
# ============================================================
...
# Drop any rows that failed to parse (rare but possible)
good_mask = ~times_utc.isna()
if not good_mask.all():
    print(f"WARNING: dropping {int((~good_mask).sum())} unparseable Times entries")
    ds = ds.isel(Time=np.where(good_mask.values)[0])
    raw_times = ["".join(t.astype(str)) for t in ds["Times"].values]
...
dx_m = float(ds.attrs.get("DX", 0.0))
dx_km = dx_m / 1000.0 if dx_m else 0.0
date_start = times_utc[0].strftime("%Y-%m-%d %H:%M UTC") if nframes else "N/A"
date_end = times_utc[-1].strftime("%Y-%m-%d %H:%M UTC") if nframes else "N/A"
...
def make_title(fig, variable_title: str):
...
generated: list[dict] = []  # {type, file, title}
...
if ds.sizes.get("Time", 0) < 2:
    # If we only have one frame, we can't compute a difference.
    rain_acc = np.zeros_like(rain.isel(Time=0).values)
else:
...
ts_label = times_local[-1].strftime("%Y-%m-%d %H:%M local") if nframes else "N/A"
...
if nframes < 2:
    print("  Not enough frames for animation (nframes < 2). Skipping T2 animation.")
else:
...
if nframes < 2:
    print("  Not enough frames for animation (nframes < 2). Skipping wind animation.")
else:
